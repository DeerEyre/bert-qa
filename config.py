...
class ModelConfig:
    def __init__(self):
        ...
        # self.model_name_or_path =  os.path.join(self.model_dir, 'pytorch_model.bin')
        self.model_name_or_path =  os.path.join(self.project_dir, 'model', 'bert-base-chinese')
        self.config_name = None
        # self.config_name = os.path.join(self.model_dir, 'config.json') # "Pretrained config name or path if not the same as model_name"
        self.tokenizer_name = None # Pretrained tokenizer name or path if not the same as model_name"
        # self.tokenizer_name = os.path.join(self.model_name_or_path, 'vocab.txt')
        self.do_train = False
        self.do_eval = False
